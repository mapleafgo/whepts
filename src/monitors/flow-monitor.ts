...
export class FlowMonitor {
    ...
    try {
      ...
      stats.forEach((stat) => {
        if (stat.type === 'inbound-rtp') {
          ...
          else if (inboundRtp.kind === 'audio' && inboundRtp.bytesReceived !== undefined) {
            audioBytes = inboundRtp.bytesReceived
          }
        }
      })

      return videoBytes > 0 ? videoBytes : audioBytes
    }
    catch {
      return 0
    }
  ...
  private triggerStreamError(): void {
    const error = new WebRTCError(
      ErrorTypes.CONNECT_ERROR,
      '数据流中断：检测到流已停滞',
    )
    this.options.emitter.emit('error', error)
  }
}
